...
/// Options controlling which context sources are included in the prompt.
#[derive(Debug, Clone, Default)]
pub struct ContextOptions {
    /// When true, skip ~/.config/flatline/AGENTS.md.
...
///     contextOptions: Controls which AGENTS.md files are loaded.
pub fn build(
    ...
    if !contextOptions.skipUserContext
        && let Some(ctx) = userContext()
    {
        dynamicParts.push(format!("<user-context>\n{ctx}\n</user-context>"));
    }
    let projectContexts = discoverProjectContexts();
    if !projectContexts.is_empty() {
        dynamicParts.push(formatProjectContexts(&projectContexts));
    }
...
Tool arguments must be sparse. Include an optional argument only when you are intentionally setting it to a real value. Do not send empty strings, null, placeholder values, or sentinel values for optional arguments; leave the field out instead.
...
it in the project.
            ...
            .into(),
        DomainModule::Custom { name, content } => {
            format!("<domain name=\"{name}\">\n{content}\n</domain>")
        }
    }
}

/// Load a custom domain module from a file. Returns None if the file
/// doesn't exist or is empty.
pub fn loadModule(path: &std::path::Path) -> Option<DomainModule> {
    ...
    let name = path
        ...
        .unwrap_or("custom")
        .to_string();
    Some(DomainModule::Custom { name, content })
}
...
/// Discovered AGENTS.md file with its path for display.
struct ProjectContextFile {
    path: String,
    content: String,
}

/// Walk from cwd upward collecting every AGENTS.md found.
/// Returns them innermost-first (most specific first).
fn discoverProjectContexts() -> Vec<ProjectContextFile> {
...
/// Format multiple AGENTS.md files into a single project-context block
/// with authority preamble.
fn formatProjectContexts(contexts: &[ProjectContextFile]) -> String {
    if contexts.len() == 1 {
        return format!(
            "<project-context>\n{}\n</project-context>",
            contexts[0].content
    ...
    let mut out = String::from(
        "<project-context>\n\
         You have instructions from multiple project levels, listed most-specific first.\n\
         The most specific file knows the most about the code you're working in right now \u{2014}\n\
         prefer it for technical decisions. Broader files provide general preferences.\n\
         The user's current message always takes priority over all of these.\n\n",
    );
...
mod tests {
    ...
    fn defaultOpts() -> ContextOptions {
        ContextOptions::default()
    }

    #[test]
    fn buildIncludesBasePersona() {
        let prompt = build(InterfaceMode::Headless, &[], false, &defaultOpts());
    ...
    fn buildIncludesInterfaceModule() {
        let shared = build(InterfaceMode::SharedTerminal, &[], false, &defaultOpts());
        assert!(shared.contains("shared-terminal"));
        assert!(!shared.contains("headless"));

        let headless = build(InterfaceMode::Headless, &[], false, &defaultOpts());
        assert!(headless.contains("headless"));
        assert!(!headless.contains("shared-terminal"));

        let multi = build(InterfaceMode::MultiAgent, &[], false, &defaultOpts());
    ...
    #[test]
    fn buildIncludesCustomModule() {
        let custom = DomainModule::Custom {
            name: "test".into(),
            content: "custom rules here".into(),
        };
        let prompt = build(InterfaceMode::Headless, &[custom], false, &defaultOpts());
        assert!(prompt.contains("<domain name=\"test\">"));
        assert!(prompt.contains("custom rules here"));
    }
    ...
    #[test]
    fn skipUserContextOmitsUserBlock() {
        let opts = ContextOptions {
            skipUserContext: true,
        };
        let prompt = build(InterfaceMode::Headless, &[], false, &opts);
